...
Windows 10 Team 2020 Update 支持基于 Chromium (版本 85 及以上) 的新 Microsoft Edge 作为 Surface Hub 2S 和 Surface Hub (v1) 的推荐浏览器。 本文介绍了如何使用以下三种方法之一安装浏览器：预配包、Microsoft Intune 或 MDM (第三方移动设备) 提供程序。
...
2. [将 Microsoft Edge 安装程序作为业务线应用添加到 Microsoft Intune。](https://docs.microsoft.com/mem/intune/apps/lob-apps-windows)
    - 如果选择使用 Microsoft Edge 更新来处理 Microsoft Edge 的自动更新，请务必配置"忽略 **应用** 版本"设置" **应用信息"** 窗格。 将此设置切换为 **"是**"时，Microsoft Intune 不会强制执行 Surface Hub 设备上安装的应用版本。
...
| 策略设置                                                                                                   | 推荐体验                                                                                                                                                                                                                                               | 默认值 |
| ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| [AutoImportAtFirstRun](https://docs.microsoft.com/deployedge/microsoft-edge-policies#autoimportatfirstrun)             | 不要自动从 Microsoft Edge 旧版导入数据类型和设置。 这可以避免使用 Surface Hub 中的共享设置更改已登录用户的配置文件。                                                                                                 | 4                 |
...
| [BrowserSignin](https://docs.microsoft.com/deployedge/microsoft-edge-policies#browsersignin)                           | 使用户能够在 Microsoft Edge Sign-On (单个) SSO 服务。 当用户登录到 Surface Hub 时，其凭据可以流动到支持的网站，而无需重新进行身份验证。  | 1                 |
...
| [ProActiveAuthEnabled](https://docs.microsoft.com/deployedge/microsoft-edge-policies#proactiveauthenabled)             | 允许 Microsoft Edge 使用 Microsoft 服务主动验证已登录用户的身份。 这简化了单一Sign-On (SSO) 体验。                                                                                                                         | 1                 |
...
> [!IMPORTANT]
> Windows 10 团队 (当前不支持) PBA 应用的可部署渐进式 Web 应用。  另请注意，Surface Hub 不支持 Microsoft Edge 策略设置[WebAppInstallForceList。](https://docs.microsoft.com/deployedge/microsoft-edge-policies#webappinstallforcelist) 
...
- [支持 ADMX (MDM) 首选](https://docs.microsoft.com/deployedge/configure-edge-with-mdm)移动设备管理提供程序，或
...
如果你使用的是默认"开始"菜单布局，可以使用 Microsoft Edge 预配包安装"开始"菜单，以将 Microsoft Edge 添加为固定应用。
如果要应用自定义的"开始"菜单布局，请使用以下 XML 为 Microsoft Edge 添加固定磁贴。
