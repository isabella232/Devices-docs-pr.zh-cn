...
7. 完成配对操作。 
8. 如果配对不成功，请再次尝试配对笔。 如有必要，请重新启动设备，然后重试。
...
> <i>Surface Hub 2 指纹读取器将立即可供购买。 有关详细信息，请查看此页面，包括如何购买。</i>
...
> [!IMPORTANT]
> 在 Windows 10 版本2004及更高版本中，默认情况下不会安装使用 Miracast 的无线投影的 Connect 应用，但可用作可选功能。 若要安装应用，请选择 "**设置**  >  **应用**"  >  **可选功能**  >  **添加功能**，然后安装**无线显示器**应用。

1. 搜索 **连接**。
2. 打开应用，然后将其关闭 (**Project 到此电脑** 可能无法正常工作，除非该应用至少已运行一次) 。
3. 点击并按住或右键单击以固定到任务栏。
4. 搜索 **投影设置**。
5. 在 **某些 Windows 和 Android 设备**下，如果你认为它是正常的，请选择 "任何地方都可用"，如果设备不在公司网络上，请选择 " **所有地方可用** "。 否则，您可以选择 **"在安全网络上的所有位置均可用"**。
6. 在 " **要求在这台电脑上投影**" 下，选择 " **仅首次**"。
7. 在 " **需要 PIN 才能进行配对**" 下，选择 " **从不**"。
