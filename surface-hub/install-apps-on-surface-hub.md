...
- 将应用提交到 Microsoft Store 时，开发人员需要设置设备系列可用性和组织授权选项，以确保应用可在 Surface Hub 上运行。
...
| MDM 提供程序                | 支持脱机授权的应用包 |
|-----------------------------|----------------------------------------|
| 配置管理器的本地 MDM （从版本1602开始） | 是 |
|
| 第三方 MDM 提供程序    | 查看以确保 MDM 提供程序支持部署脱机许可的应用包。 |

**使用 Microsoft 终结点配置管理器远程部署应用**

> [!NOTE]
> 这些说明基于 Microsoft 终结点配置管理器的当前分支。

1. 将 Surface Hub 注册到 Configuration Manager。 有关详细信息，请参阅[将 Surface Hub 注册到 MDM](manage-settings-with-mdm-for-surface-hub.md#enroll-into-mdm)。
...
12. 将应用程序部署到 Surface Hub。 有关详细信息，请参阅[通过 Microsoft 终结点配置管理器部署应用程序](https://docs.microsoft.com/sccm/apps/deploy-use/deploy-applications)。
13. 根据需要更新应用，方法是从适用于企业的应用商店下载新程序包，然后在 Configuration Manager 中发布应用程序修订。 有关详细信息，请参阅[用 Microsoft 终结点配置管理器更新和注销应用程序](https://technet.microsoft.com/library/mt595704.aspx)。 

> [!NOTE] 
> 如果您使用的是 Microsoft 终结点配置管理器（当前分支），则可以通过将 Business Store 连接到 Configuration Manager 来绕过上述步骤。 通过执行此操作，你可以将你购买的应用列表与配置管理器同步，在 Configuration Manager 控制台中查看这些应用，并像部署任何其他应用一样进行部署。 有关详细信息，请参阅[通过 Configuration Manager 管理 Microsoft Store For Business 中的应用](https://technet.microsoft.com/library/mt740630.aspx)。


## 摘要

有几种不同的方法可以在 Surface Hub 上安装应用，具体取决于你是在开发应用、评估适用于较少设备的应用，还是将应用广泛地部署到你的组织。 此表总结了受支持的方法：
