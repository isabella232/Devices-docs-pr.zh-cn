...
description: 你可以通过设置维护窗口、推迟更新或使用 Windows Server Update Services (WSUS) 来管理 Microsoft Surface Hub 或 Surface Hub 2 上的 Windows 更新。
...
为了确保设备在工作时间内始终可用，Surface Hub 在指定维护窗口期间执行其管理功能。 在 "维护" 窗口期间，Surface Hub 将通过 Windows 更新或 WSUS 自动安装更新，并在窗口结束之前的20分钟内重新启动设备。
