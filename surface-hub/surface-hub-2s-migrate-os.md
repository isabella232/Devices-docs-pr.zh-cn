...
Surface Hub 2S 附带Windows 10 协同版安装。 此自定义版本的Windows 10促进在会议室环境中进行协作。 你现在可以运行 Windows 10 专业版 或 Enterprise 来使用 Surface Hub 2S，这非常像任何其他电脑。

> [!IMPORTANT]
> 此迁移过程需要您遵循本文中描述的特定过程。 在继续之前，请阅读 [解决方案组件](#solution-components) 和 [迁移和安装工作流](#migration-and-installation-workflow-summary)。

> [!NOTE]
> 在 Windows 10 专业版 2S Enterprise安装 Surface Hub 或 Windows 10 协同版 时，需要与设备提供的现有 Windows 10 协同版 许可证不同的新许可证。
...
- Windows 10 专业版或Enterprise操作系统映像版本 1909 或更高版本
- 两个存储为 16 GB、FAT32 格式的 USB 驱动器
- Windows 10 专业版 2 Microsoft Windows Installer Enterprise MSI) Surface Hub 文件中用于 Windows 10 专业版 和 (的驱动程序和固件
- Internet 连接
- 映像解决方案 (可选) 
...
| 5 | [使用 U 盘映像Windows 10 SEMM 程序包以及驱动程序和固件加载 U 盘。](#load-a-usb-flash-drive-with-a-windows-10-image-semm-package-and-surface-hub-2-drivers-and-firmware) | 创建包含 U 盘映像Windows 10 U 盘。 此示例中，驱动器名为 *BOOTME*。 从步骤 2) 向 Surface Hub 2 (上的 Windows 10 专业版 和 Enterprise 操作系统添加驱动程序和固件 (从步骤 4) 到 *BOOTME* 驱动器。                                                                                                                                                                                                  |
...
| 7 | [安装Windows 10 专业版或Enterprise。](#install-windows-10-pro-or-enterprise)                                        | 使用 *BOOTME* 驱动器安装 Windows 10 专业版 或 Enterprise 版本 1909 或更高版本。                                                                                                                                                                                                                                                                                 |
| 8 | [安装用于客户端和Windows 10 专业版的Enterprise。](#install-surface-hub-2-drivers-and-firmware)                                        | 若要确保你的设备具有所有最新的更新和驱动程序，请安装 <a href="https://www.microsoft.com/download/details.aspx?id=101974" target="_blank">Windows 10 专业版 和 Enterprise OS 的驱动程序和固件Surface Hub 2 MSI 文件。</a>                                                                                                                                                                                                                                                                                  |
| 9 | [将 Surface Hub 2S 配置为个人工作效率设备。](#configure-recommended-settings)                                        |  启用推荐的设置和应用程序，以将 Surface Hub 2S 优化为个人生产力设备。                                                                                                                                                                                                                                                                    |
...
#### <a name="update-uefi-via-windows-update"></a>通过 Windows Update 更新 UEFI
...
4.  按照说明将映像下载到格式化的 USB 驱动器上，Windows 10 协同版 2020 Update。
5.  更新后，设备在 OOBE (进入) 体验。 无需完成设置。 UEFI 版本已更新。 而是按住电源按钮关闭设备，直到屏幕关闭。

### <a name="download-surface-uefi-configurator-and-surface-hub-2-drivers-and-firmware"></a>下载 Surface UEFI 配置器Surface Hub 2 个驱动程序和固件
...
1. 若要允许安装 Windows 10 专业版 或 Enterprise，请打开 **EnableOsMigration**，然后选择"下一步 **"**。
...
将设备注册到 SEMM 会影响管理设备。 例如，应用 SEMM 程序包后，所有 UEFI 设置 (设备 UEFI 菜单中) 锁定的 UEFI 设置。 其他设置（如 **用于 PXE 启动的 IPv6）的** 默认值也不可用。

若要在迁移完成后更改 UEFI 设置，请应用另一个 SEMM 程序包，或者从 SEMM 注销设备。 如果应用另一个 SEMM 程序包来更改 UEFI 设置，则必须在生成新的 SEMM 程序包时使用原始证书。 使用 UEFI 配置器工具，将 **EnableOSMigration** ** (保持打开状态，而不是像原始** 迁移步骤) 。
...
可以使用以下Windows 10 专业版Enterprise安装 (*版本 1909* 或更高版本) 或映像：
...
- 包含 U 盘或Windows 10 专业版Enterprise U 盘。 在安装 OOBE Wi-Fi后，此选项才 (OOBE) 可用。 设置完成后，在设备上安装[Surface Hub 2](https://www.microsoft.com/download/details.aspx?id=101974) 个驱动程序和Windows 10 专业版Enterprise和固件。
...
1. 若要创建Windows 10 专业版安装，请按照下载媒体创建工具中的说明[Windows 10。](https://www.microsoft.com/software-download/windows10) 若要下载Windows 10 企业版，请转到 [Microsoft 批量许可服务中心](https://www.microsoft.com/licensing/servicecenter/default.aspx)。
...
6. 将 MSI 文件 Surface Hub) 上的 Windows 10 专业版 2 (上的 Windows 10 专业版 和 Enterprise 操作系统的 SEMM 程序包文件和驱动程序和固件复制到包含 Windows 10 映像的 U 盘 (*BOOTME*) 的根目录。 BOOTME USB 驱动器将包含：
   ...
   - Windows 10 专业版 2 (SurfaceHub2S_Win10_18362_20.082.25682.0.msi Enterprise 上的 Surface Hub 操作系统的驱动程序和) 。 将此文件复制到 USB 驱动器的根目录。
...
7. 输入双字符证书指纹和 UEFI 设置密码。 然后选择" **确定"**。
   ...
   > [!NOTE]
   > 在设备上激活 SEMM 后，将应用新的 UEFI 设置 **EnableOSMigration** 。 您无法再访问Windows 10 协同版。 相反，您必须继续执行下一步，并安装 Windows 10 专业版 或 Windows 10 企业版。
...
3. 如果 Surface 设备没有自动启动到 U 盘，请断开设备电源 (拔下电缆，然后重新插入) 。 再次插入电缆后，设备应在几秒钟后启动。 然后，你将在屏幕中间看到白色徽标。

    如果设备未打开，请按并释放电源按钮。 在看到屏幕中间的徽标后，立即长按"音量降低"按钮，直到看到白色徽标下方的旋转圆圈。
 
   ![音量和电源按钮的图片。](images/shm-fig26.png)
   
4. 启动 OOBE (OOBE) 时，请按照说明安装 Windows 10 专业版 或 Enterprise (版本 1909 或更高版本) 。
...
若要将 Surface Hub 2S 配置为个人工作效率设备，请参阅 Windows 10 专业版 [2 Enterprise 2 Surface Hub配置 2S](surface-hub-2-post-install.md)。

> [!NOTE]
>如果无法按照本文中概述的步骤将设备成功迁移到 Windows 10 专业版 或 Enterprise for Surface Hub 2，请与支持Surface Hub[联系](https://support.microsoft.com/help/4037644/surface-contact-surface-warranty-and-software-support)。
...
> [!NOTE]
> 在回滚到Windows 10 协同版，我们建议你先从 SEMM 取消Surface Hub注册。 若要了解更多信息，请参阅 [从 SEMM 注销 Surface 设备](/surface/unenroll-surface-devices-from-semm)。
