...
###  <a name="manage-updates-with-configuration-manager-and-intune"></a>管理配置管理器和 Intune 的更新
...
###  <a name="manage-updates-with-microsoft-deployment-toolkit"></a>通过 Microsoft 部署工具包管理更新

Microsoft 部署工具包 (MDT) 包含在终结点配置管理器中。 它包含你可能希望使用的可选部署工具，具体取决于你的环境。 其中包括 Windows 评估和部署工具包 (Windows ADK) 、Windows 系统映像管理器 (Windows SIM 卡) 、部署映像服务和管理 (DISM) 以及用户状态迁移工具 (USMT) 。 您可以从 " [Microsoft 部署工具包下载" 页面](https://www.microsoft.com/download/details.aspx?id=54259)下载最新版本的 MDT。
...
终结点配置管理器和 MDT 均在部署过程中使用 Windows 预安装环境 (WindowsPE) 。 WindowsPE 仅支持一组有限的基本驱动程序，例如用于网络适配器和存储控制器的驱动程序。 不属于 WindowsPE 的 Windows 组件的驱动程序可能会产生错误。 最佳做法是将部署过程配置为仅在 WindowsPE 阶段使用所需的驱动程序，从而阻止此类错误。
...
可下载的 .msi 文件可用于 Surface Pro 2 及更高版本的设备， (在 ARM) 上运行 Windows 10 的 Surface Pro X 除外。

##  <a name="managing-firmware-with-dfci"></a>通过 DFCI 管理固件

通过让设备固件配置界面 () 内置于 Intune 中的配置文件 (现在在 [公共预览版](https://docs.microsoft.com/intune/configuration/device-firmware-configuration-interface-windows)) 中提供，Surface UEFI 管理将新式管理堆栈扩展到 UEFI 硬件级别。 DFCI 支持零接触预配，消除 BIOS 密码，提供了安全设置 (，包括启动选项和内置外围设备) ，并为将来的高级安全方案奠定了基础。 有关详细信息，请参阅以下文章：
...
##  <a name="best-practices-for-update-deployment-processes"></a>更新部署过程的最佳做法
...
###  <a name="downloading-.msi-files"></a>下载 .msi 文件
...
###  <a name="surface-.msi-naming-convention"></a>Surface 命名约定

自2019年8月起，.msi 文件使用以下命名约定：

- *产品*_*windows 发布*_*windows 内部*版本号_*版本号版本版本号*_* (通常为零) *。
...
###  <a name="legacy-surface-.msi-naming-convention"></a>旧式图面。 msi 命名约定

旧 .msi 文件 (在2019年8月之前生成的文件) 使用了相同的总体命名公式，但使用了不同的方法派生版本号。
...
- **版本：** 1900307-这将显示文件的创建日期及其在发布序列中的位置，如下所示：
  - **年份：** 19 (2019) 
  - **发布数：** 003 (年度的第三版) 
  - **产品版本号：** 07 (surface pro 6 正式是 surface pro 的第七个版本) 
- **版本：0的版本：** 0 (本版本的首次发布) 
