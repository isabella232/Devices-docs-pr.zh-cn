...
ms.locfileid: "12449165"
---
# <a name="surface-book-3-quadro-rtx-3000-tech-overview"></a>Surface Book 3 Quadro RTX 3000 技术概述

Surface Book NVIDIA® Quadro RTX™ 3000 GPU 支持的商业版 3 专为需要实时呈现、AI 加速、高级图形和便携外形设备中的计算性能的专业人员而构建。 Quadro RTX 3000 从根本上更改了可以使用新 3 Surface Book功能：

- **光线跟踪** - 使用 30 RT 核心以比以往更快地生成令人惊叹的呈现、设计和动画，以用于硬件加速光线跟踪。
- **人工智能** - 删除 GPU 加速 AI 的 240 个张量核心的冗余、繁琐的任务和计算密集型工作。
...
NVIDIA 开发和维护适用于 Enterprise (ODE) 的 Quadro 最优驱动程序，这些驱动程序经过调整、测试和验证，可提供企业级稳定性、可靠性、可用性以及扩展产品可用性支持。 每个驱动程序版本都涉及 2，000 多个人日测试专业应用程序测试套件和测试用例，以及 WHQL 认证。 将持续监视安全威胁，并发布定期安全更新，以抵御新发现的漏洞。 此外，Quadro 驱动程序在通过 Windows 更新发布之前，由 Surface 工程部门进行一层额外的测试。
...
|  组件                                        |  描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RT 内核                                            | 专用基于硬件的光线跟踪技术允许 GPU 使用物理上准确的阴影、反射和光线来呈现电影质量、光真对象和环境。  实时光线跟踪引擎与 NVIDIA OptiX、Microsoft DXR 和 Vulkan API 一起工作，可提供远超过使用传统呈现技术可能实现的真实级别。 RT 内核使用通过像素 (的较低数量) BVH 和光线转换函数加速边界卷层次结构。                                     |
| 增强的张量核心                               | 为深度学习矩阵算术构建的混合精度核心可提供与上一代相比的 8 倍 TFLOPS 培训。  Quadro RTX 3000 利用 240 个 Tensor Core;每个 Tensor Core 每时钟执行 64 个浮点融合相乘 (FMA) 操作，每个流式处理多处理器 (SM) 每个时钟总共执行 1，024 个单个浮点运算。 除了支持 FP16/FP32 矩阵操作之外，新 Tensor Cores 还添加了 INT8 (2，048 整数运算（每个时钟) 和实验 INT4 和 INT1 (二进制) 精度模式）。 |
| Turing optimized software                           | 深度学习框架（如 Microsoft Cognitive Toolkit (CNTK) 、Caffe2、MXNet、TensorFlow 等）可显著加快培训时间，提高多节点培训性能。 cuDNN、cuBLAS 和 TensorRT 等 GPU 加速库为深入学习推断和 High-Performance Computing (HPC) 提供更高的性能。                                                                                                                                                                                           |
...
| 计算抢占                                  | 在指令级别抢占可以更精细地控制计算任务，以防止长时间运行的应用程序占用系统资源或超时。                                                                                                                                                                                                                                                                                                                                                                                            |
| H.264、H.265 和 HEVC 编码/解码引擎         | 通过两个专用 H.264 和 HEVC 编码引擎以及独立于 3D/计算管道的专用解码引擎，使转换代码、视频编辑和其他编码应用程序的速度比实时性能更快。                                                                                                                                                                                                                                                                                                                                        |
...
| **Adobe Illustrator**                               | - 使用 GPU 加速画布更快地平移和缩放，这使图形设计人员和演示者可以流畅和交互地平移和缩小复杂的矢量图形。                                                                                                                                                                                                                                                                                             |
| **Adobe Lightroom**                                 | **Adobe-tested and approved for Surface Book 3 with Quadro RTX 3000**<br><br>- 使用 GPU 加速视口更快地编辑高 res 图像，从而可以建模更大的 3D 场景，并限制更复杂的动画。<br><br>- GPU 加速图像处理可以显著加快响应速度的调整，尤其是在 4K 或更高分辨率的显示器上。<br><br>- GPU 加速 AI 支持"增强详细信息"，用于优化 RAW 图像的精细颜色细节。 |
| **Adobe Photoshop**                                 | **Adobe-tested and approved for Surface Book 3 with Quadro RTX 3000**<br><br>- CUDA 核心加速通过 30+ GPU 加速功能（如模糊库、化化、智能锐化以及透视平滑）实现更快的编辑，使设计师和设计人员能够流畅快速地修改图像。                                                                                                                                                                              |
...
| **Adobe 管理刷**                         | **Adobe-tested and approved for Surface Book 3 with Quadro RTX 3000**<br><br>- 画图材料到 3d 模型，采用 RTX 加速立式和 Iray RTX 呈现，为交互式和批呈现工作流生成照片图像。                                                                                                                                                                                                                            |
...
| **Autodesk**<br>**AutoCad**                         | **Autodesk-tested and approved for Surface Book 3 with Quadro RTX 3000**<br><br>- 用于快速、交互式 3D 建模和设计的 GPU 加速视口图形。<br><br>- 使用默认 Arnold 呈现器进行 RTX 加速光线跟踪和 AI 反光。<br><br>- 与 2 15" Surface Book快 70% 以上。                                                                                                                                                        |
...
| **为 MicroStation**                            | **通过 Quadro RTX 3000 Surface Book 3 的经测试且批准的适用于 Surface Book 3**                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Dassault 系统**<br>**3D 体验平台** | - CATIA 交互式光线跟踪 (实时) 由 RT 核心加速。<br><br>- 与 2 Surface Book 15" 相比，Catia 的运行速度快 100% 以上。                                                                                                                                                                                                                                                                                                                                     |
| **Dassault 系统**<br>**Solidworks**             | - Solidworks 交互式光线跟踪 (可视化) RT Core 和 Tensor 核心加速;AI 加速青绿色。<br><br>- 运行速度比 2 Surface Book 15" 快 50% 以上。                                                                                                                                                                                                                                                                                             |
...
| **McNeel & Associates**<br>**Rhino 3D**             | **Rhino-tested and approved for Surface Book 3 with Quadro RTX 3000**<br><br>- GPU 加速视区，提供流畅的交互式建模和设计体验。<br><br>- 支持 GPU 加速 3D 呈现的周期。                                                                                                                                                                                                                                                         |
| **PTC Creo**                                        |**PTC 经过 PTC 测试和批准，Surface Book 3 和 Quadro RTX 3000**<br><br> - Creo 在 CUDA 上构建 (Creo Simulation Live) 实时模拟工具。<br><br>- 运行速度比 2 Surface Book 15" 快 15% 以上。                                                                                                                                                                                        |
| **SolidEdge 2020**                                        |  **使用 Quadro RTX 3000 Surface Book 3 的经测试且批准的为 Quadro**                                                                                                                                                                                 |
...
**表 4. Surface Book 3 与 Quadro RTX 3000 SKUs**
