...
命令行 | 使用标准工具（如 Configuration Manager）直接对 Surface 设备进行远程故障排除，而无需用户交互。 它包含以下命令：<br>`-DataCollector` 收集所有日志文件<br>`-bpa` 使用最佳做法分析器运行运行状况诊断。<br>`-windowsupdate` 检查Windows更新中缺少固件或驱动程序更新。<br>`-warranty` 检查担保信息。 <br><br>| SDT 控制台应用：<br>Microsoft Surface Diagnostics 应用控制台<br>[适合 IT 的 Surface 工具](https://www.microsoft.com/download/details.aspx?id=46703) | [使用命令Toolkit Surface Diagnostic Toolkit](surface-diagnostic-toolkit-command-line.md)
...
2. 从适用于 IT (.msi) 下载页面下载 SDT [Windows](https://www.microsoft.com/download/details.aspx?id=46703)安装程序程序包，并复制到 Surface 设备上的首选位置，如桌面。
...
SDT 和 SDT 应用控制台都安装在 上 `C:\Program Files\Microsoft\Surface\Microsoft Surface Diagnostic Toolkit for Business` 。
...
![选择Windows更新"选项](images/sdt-5.png)
...
- [使用适用于Toolkit Surface 诊断工具](surface-diagnostic-toolkit-command-line.md)
