...
与当今Windows 10设备相比，Surface 为 IT 管理员提供了通过一组丰富的 UEFI 配置设置配置和管理固件的能力。 这将在基于软件的策略管理的基础上提供一层硬件控制，通过移动设备管理、MDM (策略、Configuration Manager 或组策略) 实现。 例如，在具有敏感信息的高度安全区域部署设备的组织可以通过删除硬件级别的功能来阻止使用相机。 从设备的角度来看，通过固件设置关闭相机等效于以物理方式移除相机。 将固件级别的管理新增的安全性与仅依赖操作系统软件设置进行比较。 例如，如果通过域Windows策略设置禁用音频服务，本地管理员仍可以重新启用该服务。
...
尽管通常几乎会立即应用 Intune 策略设置，但设置在目标设备上生效前可能会延迟 10 分钟。 在极少数情况下，最多可能会延迟 8 小时。 若要确保设置尽快应用， (如在测试) ，你可以手动同步目标设备。
...
3. 连接 Surface 品牌以太网适配器连接 Internet 的设备。 重新启动设备并打开 UEFI 菜单 (长按调高音量按钮，同时按下并释放电源按钮) 。
